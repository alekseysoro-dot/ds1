...
- `доработка` | Keywords UI + реальный парсинг + фикс Dockerfile | sidebar: секция ключевых слов (CRUD), модалка добавления с категорией; startParsing → POST /api/parse/run; Dockerfile: COPY . ./api/, __init__.py | задеплоено, ожидается проверка
