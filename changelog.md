...
- `доработка` | реализация Telegram-уведомлений | send_telegram через httpx + Telegram Bot API, интеграция в check_price_alerts (email + TG параллельно), config.py, conftest, .env.example, 12 тестов notifier (48 total) | 48/48 тестов, тестовое сообщение доставлено в TG, settings обновлены (email + tg_chat_id + threshold=5%)
