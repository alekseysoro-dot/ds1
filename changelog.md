...
- `доработка` | мультикейворд + категории | модель Keyword, CRUD API (GET/POST/DELETE/PATCH), scheduler с перебором keywords и паузой 60с, fallback на APIFY_KEYWORD, init.sql, 9+2 тестов | 65/65 тестов, закоммичено (2e6fd2c), задеплоено, таблица keywords создана
- `баг` | price-parser.html на сервере стал пустой директорией | восстановлен из git show 45c067a, API_BASE='', API_KEY через настройки⚙→localStorage | восстановлено
- `баг` | API крашится — ModuleNotFoundError 'api' после git pull (Dockerfile COPY . . вместо COPY . ./api/) | sed исправлен Dockerfile, touch __init__.py, docker compose up --build | исправлено, API работает
- `баг` | фронтенд: нет текущей/мин цены, undefined в колонке товара | переписан loadProducts: маппинг API→фронтенд формат, загрузка цен через /api/prices/{id}, _timeAgo хелпер, API_BASE='' | исправлено, цены и сетка в норме
- `вопрос` | в колонке «Изменения %» нет данных | delta_pct=null т.к. только одна запись цены; после повторного парсинга данные появились | исправлено
- `вопрос` | как посмотреть лимиты Claude Code | команда `/usage` или `claude --usage` | ответ дан

## 2026-04-05

- `вопрос` | возвращение пользователя, статус проекта | напомнен статус: всё работает, мультикейворд API готов, открыты задачи фронтенда | ожидается решение
- `баг` | нет уведомлений на почту и Telegram | таблица notification_settings была пустая; chat_id=531342852, email, threshold=5% добавлены в БД | Telegram работает
- `баг` | email не отправляется — SMTPAuthenticationError 535 | создан новый пароль приложения Gmail (Parser_new), обновлён в .env, docker compose перезапущен | исправлено, email и Telegram работают
- `вопрос` | лимит 15%, фронтенд откладываем | фиксируем текущий статус, фронтенд (keywords UI, поиск, категории) + коммит серверных файлов — на следующую сессию | отложено

## 2026-04-06

- `вопрос` | возвращение пользователя | напомнен статус, задачи: фронтенд + коммит серверных файлов | начали с фронтенда
- `доработка` | Keywords UI + реальный парсинг + фикс Dockerfile | sidebar: секция ключевых слов (CRUD), модалка добавления с категорией; startParsing → POST /api/parse/run; Dockerfile: COPY . ./api/, __init__.py | задеплоено
- `баг` | "Обновить цены" не использовал keywords из таблицы | startParsing теперь итерирует все активные keywords с паузой 60с, retry на 503 | закоммичено (569132e)
- `баг` | git reset --hard вернул docker-compose.yml с nginx | nano: убран nginx, добавлен core-net, контейнеры запущены | исправлено
- `доработка` | проверка Keywords UI | ключевое слово добавлено, товар в общем списке но не в категории — group_name не привязывается при парсинге | нужна доработка
- `баг` | товары не попадают в категорию при парсинге по ключевому слову | parse.py и scheduler.py: _save_results/_save_prices теперь получают group_name из keywords.category и ставят его на новые Product | закоммичено (ae3ff0c), ожидается деплой на сервер
- `вопрос` | будут ли работать только товары с фронтенда | объяснено: категория ставится на новые товары при парсинге, старые без категории можно удалить через SQL и пересоздать парсингом | ответ дан
- `доработка` | удалена кнопка "Добавить товар" и mock-логика (модалка, фид-импорт) | единственный путь добавления товаров — через ключевые слова с категориями, удалено ~185 строк мёртвого кода | закоммичено (5eac66c), ожидается деплой
- `деплой` | инструкция по деплою: git pull + HTML + rebuild + restart caddy | кнопка убрана, фронтенд обновлён | выполнено
- `баг` | категории пустые — товары без group_name | старые товары удалены, после парсинга пересозданы с group_name="Бытовая техника" | исправлено
- `доработка` | добавлены категории "Здоровье" и "Техника" в фильтры и модалку ключевых слов | 2 места: чипы сайдбара + select модалки | задеплоено, работает

## 2026-04-09

- `вопрос` | возвращение пользователя | напомнен статус проекта, предложены следующие шаги | ожидается выбор задачи
- `инфраструктура` | синхронизация docker-compose.yml с сервером | убран nginx, добавлен core-net — git pull теперь проходит без конфликтов | выполнено (e77ec24)
- `доработка` | drawer: реальные данные продавцов вместо mock | секция "Цены у продавцов" теперь загружает из /api/prices/{id}, сортировка по цене, показ delta% | задеплоено, git pull прошёл чисто
- `баг` | фильтр stocks отсёк все товары — WB search API не включает stocks | откачен фильтр, задеплоено, 100 товаров возвращается | исправлено (5006f9c)
- `доработка` | favicon для сайта | inline SVG "PP" в стиле логотипа (тёмный фон, голубая рамка) | закоммичено (cc021b9), ожидается деплой
- `доработка` | сортировка таблицы по колонкам | клик на "Текущая цена", "Мин. цена", "Изменение" — toggle desc/asc, стрелки-индикаторы | задеплоено, работает
- `вопрос` | аудит настроек — что работает, что нет | Email/TG/Порог — одна строка в БД, замена а не добавление; объяснено | ожидается подтверждение на подключение к API

## 2026-04-17

- `вопрос` | возвращение пользователя | напомнен статус, предложено продолжить подключение настроек к API | задача другая
- `доработка` | каскадное удаление товаров при удалении ключевого слова | DELETE keyword теперь удаляет products + sellers + price_history с matching group_name; остатки (416 товаров) очищены вручную | выполнено (9e2b904)

## 2026-04-23

- `баг` | уведомления о ТВ-товарах продолжают приходить после очистки БД | анализ кода: check_price_alerts() в notifier.py сканирует ВСЕХ sellers независимо от keywords — уведомления идут по любым товарам в базе; запрошена диагностика: keywords, распределение products по group_name, TV-товары | ожидается вывод с сервера
- `инфраструктура` | SSH к серверу требует подтверждения fingerprint при первом подключении | выдан вариант команды с `-o StrictHostKeyChecking=accept-new` для автопринятия | ожидается выполнение
- `баг` | диагностика: keywords = 0 строк, но уведомления идут | подтверждено — уведомления не зависят от keywords; вторая диагностика (products/sellers/price_history, распределение по group_name) с правильным экранированием через `;` и `'"'"'` | ожидается вывод
- `баг` | найдена корневая причина: scheduler использовал APIFY_KEYWORD как fallback при пустой таблице keywords | удалён fallback в scheduler.py:33-37, убран неиспользуемый импорт settings, обновлён test_falls_back_to_env_keyword → test_skips_when_no_keywords; коммит 560d056, запушено в origin+parsernew | ожидается деплой: git pull + docker compose up -d --build api + DELETE FROM price_history/sellers/products (131 товар, 1100 prices)
- `инфраструктура` | путь `/root/ds1` на сервере не существует | запрошен поиск docker-compose.yml в /root /opt /srv /home + вывод docker ps | найдено /opt/core-stack/docker-compose.yml (caddy/redis/ollama), backend-api-1 и backend-db-1 работают, но compose-файл price-parser не в стандартных путях
- `инфраструктура` | поиск git-клона price-parser на сервере | запрошен `docker inspect backend-api-1` (working_dir) + `find / -name docker-compose.yml` | найдено: проект в /home/ds1, compose в /home/ds1/reverens/backend/docker-compose.yml — путь зафиксирован
- `деплой` | применение fix для уведомлений | выдана команда: cd /home/ds1 && git pull + docker compose up -d --build api + DELETE products/sellers/price_history | выполнено: commit 560d056 задеплоен, образ пересобран, backend-api-1 started; БД очищена (DELETE 1100 prices, 131 sellers, 131 products). Уведомления должны прекратиться — следующий тик scheduler'а найдёт пустые keywords и пропустит парсинг
- `инфраструктура` | проверка через $()-substitution в SSH сломалась на парсинге .env | упрощён запрос до `docker logs backend-api-1 --tail 30` | контейнер стартовал чисто: "Application startup complete", Uvicorn на :8000, без ошибок. Задача закрыта — 3 слоя защиты: keywords=0, scheduler skip при empty, products=0
